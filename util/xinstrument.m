...
obs = { ...
	...
	'zero', 'zero', @(~,r) zeros(r.d.a), [], [], 'square modulus of field'; ...
	...
	'g2raw', 'g2raw', @(a,r) abs(a).^4 - 2*abs(a).^2/r.dV + 1/(2*r.dV^2), [], 2, 'pair correlations'; ...
...
for o = varargin
	...
	if ischar(o)
		...
		if strcmp(o, 'g2tw')
			if in.ensembles(1) < 100
				warning(sprintf( ...
'g2 is being normalised with ensembles(1)=%d, this is too small for the statistics to be meaningful.', ...
				 in.ensembles(1)))
			end
